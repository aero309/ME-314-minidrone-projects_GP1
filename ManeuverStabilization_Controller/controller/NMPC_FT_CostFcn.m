function J = NMPC_FT_CostFcn(X,U,e,data)
    ...
    for i = 1: N-1
        ...
        current_y = [X(i,1:3), qxy(2)^2+ qxy(3)^2, qz(4), X(i,8:13), U(i, :)];
        ...
        current_y = [X(N,1:3), qxy(2)^2+ qxy(3)^2, qz(4), X(N,8:13), U(N, :)];
        yref = [data.References(N, 1:3), 0, 0, data.References(N, 4:9) ,tref]; %add 2 zeros
        y = current_y - yref;
        J = y*Q*y';
end
